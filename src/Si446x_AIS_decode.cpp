  ...
  if (digitalRead(CLSpin))
  {
    ...
    case PH_STATE_WAIT_FOR_SYNC: // state: waiting for preamble and start flag
      ...
      case PH_SYNC_FLAG: // sub-state: start flag detection
        ...
        if (rx_sync_count != 0)
        {                                  // if this is not the last bit of start flag
    ...
    case PH_STATE_RECEIVE_PACKET: // state: receiving packet data
      ...
      if (rx_one_count == 5)
      { // if we expect a stuff-bit..
        if (rx_bit)
        {                                    // if stuff bit is not zero the packet is invalid
      ...
      if ((rx_bitstream & 0xff00) == 0x7e00)
      {                                 // if we found the end flag 0x7e we're done
      ...
      if (rx_bit_count > 1020)
      {                                 // if packet is too long, it's probably invalid
    ...
    if (ph_state == PH_STATE_RESET)
    {                        // if next state is reset
